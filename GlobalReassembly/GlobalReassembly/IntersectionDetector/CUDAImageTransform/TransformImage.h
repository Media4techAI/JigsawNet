...
extern "C" uint8 *TransformImage(uint8 *src, int src_rows, int src_cols, uint8 *dst, int dst_rows, int dst_cols, double *affine_mat, int &out_rows, int &out_cols, double &out_overlap_ratio, int &offset_row, int &offset_col, int &out_overlap_pixels);
extern "C" void DeleteFusionImage(uint8 *fusion_image);

extern "C" void OnlyCalculateIntersection(uint8 *src, int src_rows, int src_cols, uint8 *dst, int dst_rows, int dst_cols, double *affine_mat, int &out_overlap_pixels, double &out_overlap_ratio);
#else
