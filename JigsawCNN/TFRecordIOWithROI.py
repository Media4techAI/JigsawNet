...
def createTFRecord(tfrecords_filename, dataset_root=Parameters.WorkSpacePath['training_dataset_root']):
    ...
    print(os.path.join(dataset_root, "jigsaw_training", "*"))
    ao = os.path.join(dataset_root, "jigsaw_training", "*")
    training_data_path1 = glob.glob(os.path.join(dataset_root, "jigsaw_training", "*"))
...
def readTFRecord(filename_queue):
    print("📥 [readTFRecord] Starting TFRecord read")

    reader = tf.TFRecordReader()
    print("📥 [readTFRecord] TFRecordReader created")

    fn, serialized_example = reader.read(filename_queue)
    print("📥 [readTFRecord] reader.read called")
    ...
    print("📥 [readTFRecord] Features parsed")

    training_input = tf.decode_raw(features['training_input'], tf.uint8)
    print("🧱 [readTFRecord] Decoded training_input")

    height = tf.cast(features['height'], tf.int32)
    width = tf.cast(features['width'], tf.int32)
    data_id = tf.cast(features['id'], tf.int32)
    print("📐 [readTFRecord] Cast height, width, data_id")

    training_target = features['training_target'].values
    training_roi = features['training_roi'].values
    print("🎯 [readTFRecord] Extracted target and ROI values")

    input_shape = tf.stack([
        Parameters.NNHyperparameters['height'],
        Parameters.NNHyperparameters['width'],
        Parameters.NNHyperparameters['depth']
    ])
    print("📦 [readTFRecord] Input shape created:", input_shape)
    ...
    print("🔄 [readTFRecord] Reshaped input, target, ROI, and ID")

    batch_size = Parameters.NNHyperparameters['batch_size']
    print(f"🧪 [readTFRecord] Creating shuffle_batch (batch_size={batch_size})")

    inputs, targets, roi_boxes, data_ids = tf.train.shuffle_batch(
        [training_input, training_target, training_roi, data_id],
    ...
    print("✅ [readTFRecord] shuffle_batch created")

    inputs = tf.cast(inputs, tf.float32)
    targets = tf.cast(targets, tf.float32)
    roi_boxes = tf.cast(roi_boxes, tf.float32)
    data_ids = tf.cast(data_ids, tf.int32)
    print("🔁 [readTFRecord] Casted tensors to final types")
